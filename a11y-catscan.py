...
# JavaScript element resolver — runs in the live DOM after all engines
# scan each page, normalizing element references to uniform CSS selectors.
#
# Each engine identifies elements differently:
#   axe:   CSS selector ('#hero', 'div > p.intro:nth-child(2)')
#   IBM:   XPath ('/html[1]/body[1]/main[1]/p[1]')
#   HTMLCS: CSS selector (generated by our _sel() in the HTMLCS eval)
#   Alfa:  tag + attributes from normalizeOutcome in alfa-engine.mjs
#
# This resolver takes all these formats, finds each element in the
# live DOM, and generates a deterministic CSS selector using the same
# algorithm for all engines.  This enables cross-engine dedup: if axe
# and IBM flag the same <p>, they both get the same selector string.
#
# Performance: ~0.2ms per node, negligible vs scan time.
